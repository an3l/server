...
BEGIN
    ...
    SET out_exists = (SELECT TABLE_TYPE FROM information_schema.TABLES WHERE
                            TABLE_SCHEMA = in_db AND
                            TABLE_NAME = in_table AND
                            TABLE_TYPE= 'temporary');
    IF out_exists is NULL
    THEN
        -- It is not temporary table/sequence
        SET out_exists = (SELECT TABLE_TYPE FROM information_schema.TABLES WHERE
                                TABLE_SCHEMA = in_db AND
                                TABLE_NAME = in_table);
        IF out_exists is NULL
        ...
        IF out_exists = 'SYSTEM VERSIONED' THEN
