...
MYSQL_BINARY_LOG mysql_bin_log(&sync_binlog_period, 0);
MYSQL_RELAY_LOG mysql_relay_log(&sync_binlog_period, 1);
...
bool MYSQL_BINARY_LOG::can_purge_log(const char *log_file_name_arg)
{
...
bool MYSQL_RELAY_LOG::can_purge_log(const char *log_file_name_arg)
{
  THD *thd= current_thd;                        // May be NULL at startup
  bool res;
  if (is_active(log_file_name_arg))
    return false;
  res= log_in_use(log_file_name_arg,
                  (is_relay_log ||
                  (thd && thd->lex->sql_command == SQLCOM_PURGE)) ?
                  0 : slave_connections_needed_for_purge);
  return !res;
