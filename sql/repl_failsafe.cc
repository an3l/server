...
static my_bool show_slave_hosts_callback(THD *thd, Protocol *protocol)
{
  ...
  if (const Slave_info *si= thd->slave_info)
  {
    ...
    {
      gtid_state_from_binlog_pos(si->gtid_state_sent.log_file, si->gtid_state_sent.log_pos, gtid_sent);
      gtid_state_from_binlog_pos(si->gtid_state_ack.log_file, si->gtid_state_ack.log_pos, gtid_ack);
    }
